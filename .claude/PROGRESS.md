...
- **T-05 must call the shared CSV sanitizer — do not skip it or reimplement it.** The CSV/formula-injection defense (strips leading `=`/`+`/`-`/`@`/whitespace, including tab/CR, with bypass-resistant looping) lives in `src/lib/sanitizeCsvCell.ts`, imported by the T-02 preview screen and applied to every column via Papa Parse's `transform` option (not just the image filename). Per the Security/Trust-Boundary design, the T-02 UI check is convenience only — it is **not** a security boundary. T-05's actual insert path (the bulk-import Edge Function per `docs/API-Integration-Contracts.md`) must apply this exact same sanitization before persisting any CSV-derived value, not assume T-02 already handled it. Note: the Edge Function runs on Deno, a different runtime than this Next.js app — if a direct import of `src/lib/sanitizeCsvCell.ts` isn't practical from that function's deployment setup, T-05 must still use logic identical to it (e.g. copy it verbatim and keep both in sync), not a separately-invented sanitizer. Regression-protected by `src/lib/sanitizeCsvCell.test.ts` (Vitest, `npm test`) — covering leading `=`/`+`/`-`/`@`, leading space before those, tab/CR alone and combined, and nested/repeated runs like `" = =cmd"`. Extend this test file rather than re-verifying by hand if the logic ever changes.
