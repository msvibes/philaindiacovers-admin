...
**No bug fix, enhancement, or feature may cause a regression.** Before any PR is considered done, verify not just the new thing but that nothing adjacent broke — run the full test suite, and for anything touching shared components, shared state, or shared visual tokens, do a live pass on the adjacent features that share that surface. *Precedent: this formalizes discipline the App repo already practiced informally but inconsistently (T-30/T-35's dark-mode continuity catches, T-34's tooltip-clipping catch) — same rule, same reasoning, applied here since both repos share this document's standing-rules structure verbatim.*
...
- **When handling any Supabase API key or credential, always decode it (JWT payload) and verify its actual `role` claim before use, writing to a file, or embedding in a build — checking presence, length, or the variable's own name is not sufficient verification.** This applies to every future session, not just when something feels risky. Learned from a real incident, 2026-09-03 (App repo, but the same risk applies here — this repo's own `SUPABASE_SERVICE_ROLE_KEY` handling in `src/lib/supabaseAdminClient.ts` and `scripts/provision-user.mjs` is exactly the kind of place a role mixup would matter): during a service-role-key rotation, the new service-role key and the new anon key got crossed between two env files — a value named as the anon key was actually the service-role key, and it was built into and delivered as part of a real installer before being caught. The only verification performed up to that point was confirming a value was present and checking its length — never decoding the token itself. `scripts/check-no-secret-leak.mjs` was fixed the same day, in both repos, to decode any JWT-shaped string and check its own `role` claim directly, rather than trusting a prefix or a variable name.
