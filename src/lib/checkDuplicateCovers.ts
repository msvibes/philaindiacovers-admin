...
// has to happen server-side with the service-role key instead. Requires an
// Admin session as of T-06.5 — see authorizedFetch.ts.
export async function fetchExistingCoverKeys(
  giItemNames: string[]
): Promise<ExistingCoverKey[]> {
  if (giItemNames.length === 0) return [];

  const res = await authorizedFetch("/api/check-duplicate-covers", {
    method: "POST",
