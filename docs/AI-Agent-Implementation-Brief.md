...
| T-05 | US-36 | Wire the import screen's "confirm" action to actually create `draft` rows in `covers`. Includes: (a) extracting `gi_registration_number` by pattern-matching it out of "Name of the GI Tag / Item" text (e.g. "(GI No. 438)") — it is not its own CSV column; (b) parsing `date_of_issue` from the real spreadsheet's mixed formats, not just clean ISO dates | — | After confirming a valid test import, the new rows appear in the database with `verification_status = 'draft'`. Also: a row whose GI Tag/Item text embeds a GI number extracts it correctly into `gi_registration_number`; rows with real messy date formats from `PhilaIndiaCovers-PLabs.xlsx` (e.g. "05.09.2021") parse into a correct `date_of_issue`, not just the clean ISO dates in the T-02 test fixture |
...
| T-06.5 | US-34 (EPIC-08, Admin & Verifier Access) — "As an Admin or Verifier, I want to log into the back-office via email/password or Google SSO, so that I can access my role's tools" | **New task for an existing story, added retroactively (2026-08-04) — none of T-01–T-09 as originally decomposed ever build Admin/Verifier authentication, even though T-07 assumes a real logged-in Verifier account and T-06's role checks (`current_profile_role()`) are meaningless without one.** Build real login for the Admin back-office: a login UI (Supabase Auth, matching the email/password + Google SSO pattern the consumer app already commits to per its `CLAUDE.md`), server-side session/JWT verification in the Next.js app, and use that same session check to close the `/api/check-duplicate-covers` access-control gap open since T-03 (require a verified `role = 'admin'` session before the route runs its service-role query, instead of the route being unauthenticated) | No Verifier-facing review UI yet — that's T-07, which can now build on a real session instead of a manually-set test account. No consumer-app (Electron) Collector login — that's a separate repo/story with its own auth flow (email/password + Google SSO per the App repo's `CLAUDE.md`), not covered here; T-08/T-09 will hit an analogous "no real session to test against" gap in that repo, not addressed by this task. No password-reset/email-verification flows beyond Supabase Auth's own defaults — scope is exactly what's needed to unblock T-07 and close the `/api/check-duplicate-covers` gap, not a full auth feature set | An unauthenticated request to `/api/check-duplicate-covers` is rejected (401/403), not served. A real Verifier test account can log in through the UI and reach an authenticated session. A request to `/api/check-duplicate-covers` from a logged-in Admin session succeeds; from a logged-in Verifier or Collector session, it's rejected — confirming the check is a real server-side role verification, not a client-supplied claim |
| T-07 | US-39 | Build the Verifier's review screen: list Draft/Flagged entries, Verify/Flag-with-reason buttons calling `verify_cover()` | — | Using a real Verifier account authenticated through T-06.5's login, mark a seeded Draft entry Verified; confirm it updates immediately |
